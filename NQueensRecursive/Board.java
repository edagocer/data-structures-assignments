...
public class Board {
	...
	Board(int width, int height) {
		...
		Arrays.fill(queens, NoQueen); // tüm sütunlarda kraliçe olmadığını gösterir.
	}
	...
	public boolean addQueen(int x, int y) {
		if (isInsideBoard(x, y)) {
			queens[x] = y; //x satırındaki y sütununa kraliçe yerleştirir. 
	...
	public Integer getFirstEmptyColumn() {
		for (int x=0; x<width; x++) {
			// Bir sütunun boş olup olmadığını kontrol etmek için kullanılır.
			//Eğer tahtada boş bir sütun yoksa, çözüm tamamlanmış demektir.
			if (queens[x] == NoQueen) {
				return x;
			}
		}		
	...
	private boolean checkAttackInDirection(int x, int y, int delta_x, int delta_y) {
		...
		while (isInsideBoard(xp, yp)) {
			if (isQueen(xp, yp)) {
				return true; //başka bir kraliçe tarafından tehdit edildiği anlaşılır.
			}
	...
	public boolean canAttackOtherQueens(int x, int y) {
		...
		//true ise kraliçe başka bir kraliçe tarafından tehdit edildiği anlaşılır.
